...
class fdmViewSidebar extends fdmView {
	...
	public function set_classes( $classes = array() ) {
		...
		$classes = array_merge(
			...
			array(
				...
				'fdm-sidebar-display-' . $fdm_controller->show_sidebar,
				'fdm-sidebar-mobile-expand-' . $fdm_controller->settings->get_setting('fdm-sidebar-mobile-expand'),
				'fdm-sidebar-menu-style-' . $fdm_controller->menu_prostyle_value
			)
		);
	...
	function enqueue_assets() {
		...
		if ( $fdm_controller->show_sidebar != '1' ) :
			return;
		endif;
		...
		wp_localize_script(
			...
			array(
				...
				'menu_style' => $fdm_controller->menu_prostyle_value
			)
		);
	}
